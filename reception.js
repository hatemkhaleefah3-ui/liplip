...
const LiplipReception = (() => {
 ...
 function formatCard(kind){const watch=kind==='watch',heads=watch?LiplipReceptionImporter.WATCH_HEADERS:LiplipReceptionImporter.READ_HEADERS;return `<article class="reception-format-card"><span>${watch?'قالب المشاهدة':'قالب القراءة'}</span><strong>${watch?'watch / watchExam':'read / readExam'}</strong><p>${watch?'video · textVoice · imageArabicWrite · mcq':'storyPage · enToArWrite · arToEnWrite · mcq'}</p><button class="reception-template-download" data-reception="download-template" data-kind="${kind}">تنزيل قالب Excel</button><details><summary>أسماء الأعمدة المطلوبة</summary><code>${heads.map(safe).join(' | ')}</code></details></article>`}
 function xmlEsc(value){return String(value??'').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]))}
 function cellRef(col,row){let n=col+1,s='';while(n){const r=(n-1)%26;s=String.fromCharCode(65+r)+s;n=Math.floor((n-1)/26)}return s+row}
 async function downloadTemplate(kind){
  if(typeof JSZip==='undefined')throw Error('تعذّر إنشاء قالب Excel. أعد تحميل الصفحة.');
  const watch=kind==='watch',headers=watch?LiplipReceptionImporter.WATCH_HEADERS:LiplipReceptionImporter.READ_HEADERS;
  const samples=watch?[
   ['watch',1,1,1,'video','فيديو الصندوق','https://www.youtube.com/watch?v=VIDEO_ID','Hello there','استمع إلى العبارة','','','','','','','','',''],
   ['watchExam',1,1,1,'textVoice','','','Hello there','','','استمع ثم اختر النص المطابق','Hello there','Goodbye','Thank you','',1,'',''],
   ['watchExam',1,1,1,'imageArabicWrite','','','','','https://example.com/image.jpg','اكتب معنى الصورة بالعربية','','','','','','قطة',''],
   ['watchExam',1,1,1,'mcq','','','','','','اختر الإجابة الصحيحة','الخيار الأول','الخيار الثاني','الخيار الثالث','',1,'','شرح بالعربية']
  ]:[
   ...
   ['readExam',1,1,1,'mcq','',1,'','','','من لديه كرة؟','Tom','Sara','Ali','',1,'','شرح بالعربية']
  ];
  const rows=[headers,...samples],sheetRows=rows.map((row,ri)=>`<row r="${ri+1}">${row.map((v,ci)=>v===undefined||v===null||v===''?'':`<c r="${cellRef(ci,ri+1)}" t="inlineStr"><is><t>${xmlEsc(v)}</t></is></c>`).join('')}</row>`).join('');
  const zip=new JSZip();
  ...
  zip.folder('xl').file('workbook.xml','<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Content" sheetId="1" r:id="rId1"/></sheets></workbook>');
  ...
  const blob=await zip.generateAsync({type:'blob'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=watch?'liplip-watch-template.xlsx':'liplip-read-template.xlsx';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),0)
 }
 ...
 function click(action,target){
  ...
  if(action==='download-template')return {downloadTemplate:target.dataset.kind==='read'?'read':'watch'}
  return {}
 }
 ...
 return {PHASES,TYPES,start,setPhase,clear,render,click,submit,change,importFile,downloadTemplate,getContent,validate};
})();