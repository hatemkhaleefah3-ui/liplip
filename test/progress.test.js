...
assert.match(app.innerHTML,/status-strip/);
assert.equal((app.innerHTML.match(/class="status-upper"/g)||[]).length,1);
assert.equal((app.innerHTML.match(/class="status-lower"/g)||[]).length,1);
assert.doesNotMatch(app.innerHTML,/data-action="logout"/);
assert.ok(app.innerHTML.includes('ادخل صندوق التعلّم'));
assert.doesNotMatch(app.innerHTML,/study-next|study-footer-row|study-manage/);
assert.equal((app.innerHTML.match(/class="study-path-node /g)||[]).length,20);
assert.match(app.innerHTML,/لمحة قبل أن تبدأ/);
click('action','study-box');assert.ok(app.innerHTML.includes('كلمة جديدة'));
click('zone','exit');assert.ok(app.innerHTML.includes('ادخل صندوق التعلّم'));
assert.doesNotMatch(app.innerHTML,/study-next|study-footer-row|study-manage/);
assert.equal((app.innerHTML.match(/class="study-path-node /g)||[]).length,20);
assert.match(app.innerHTML,/لمحة قبل أن تبدأ/);
